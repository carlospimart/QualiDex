...
public class BaseImageExtraction {
    ...
    private static final Logger logger = Logger.getLogger(BaseImageExtraction.class.getName());
    private final Asserter asserter = new Asserter();

    public double imageCompare(String refImagePath, String extractedImagePath) {
        BufferedImage imgA;
        BufferedImage imgB;
        double percentage = 0;
        try {
            ...
            if ((width1 != width2) || (height1 != height2)) {
                asserter.validateFalse((width1 != width2) || (height1 != height2),"Error: " + refImagePath + " and " + extractedImagePath + " image dimensions are not matching");
            } else {
                long difference = 0;
            ...
            logger.info("Error: Either file1 or file2 images are not equivalent each other so " + e.getMessage());
        }
    ...
    public BufferedImage compareWithBaseImage(BufferedImage bImage, BufferedImage cImage) {
